    ...
    <div>
      ...
      {drafts.map((d, i) => (
        <div key={d.key} className="rounded-2xl p-3.5 mb-2.5" style={{ background: 'rgba(255,255,255,0.85)' }}>
          <div className="flex items-center justify-between mb-2">
            <div className="text-sm font-black" style={{ color: '#3A3226' }}>{i + 1}번 문제</div>
            {drafts.length > 1 && (
              <button
          ...
          <textarea
            ...
            className="w-full rounded-xl px-3 py-2 text-[15px] outline-none resize-none mb-2"
          ...
          {d.media === 'image' && (
            <div className="mb-2">
              ...
              >
                {d.uploading ? (
                  <span className="text-[13px]" style={{ color: '#A89880' }}>올리는 중...</span>
                ) : d.imageUrl ? (
            ...
            <div className="mb-2">
              ...
              {d.youtube && (
                parseYoutubeId(d.youtube) ? (
                  ...
                  <div className="text-[12px] mt-1" style={{ color: '#C0392B' }}>
                    주소를 알아볼 수 없어요
                  </div>
                )
              )}
              ...
              {d.choices.map((c, ci) => (
                <div key={ci} className="flex items-center gap-1.5 mb-1.5">
                  <button
                    ...
                    className="shrink-0 w-6 h-6 rounded-full text-[13px] font-bold"
                    style={{
                  ...
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-lg px-2.5 py-1.5 text-[14px] outline-none"
                    style={{ background: 'white', color: '#3A3226' }}
                  />
                  {d.choices.length > 2 && (
                    <button
          ...
          {d.type === 'short' && (
            <>
              ...
              <input
                ...
                className="w-full rounded-lg px-2.5 py-1.5 text-[14px] outline-none mb-1"
                style={{ background: 'white', color: '#3A3226' }}
              />
              <div className="text-[11px] mb-1" style={{ color: '#A89880' }}>
          ...
          {d.type === 'essay' && (
            <div className="rounded-lg px-2.5 py-2 mb-1 text-[12px] leading-relaxed" style={{ background: '#F6F0E4', color: '#8A7A5F' }}>
          ...
          <input
            ...
            className="w-full rounded-lg px-2.5 py-1.5 text-[13px] outline-none mt-1"
            style={{ background: 'white', color: '#3A3226' }}
          />
        </div>
      ))}
      ...
      {confirmMsg && (
        <div className="rounded-2xl p-3.5 mb-2" style={{ background: '#FFF1D6', border: '1px solid #F0D9A8' }}>
          <div className="text-[14px] font-bold mb-1" style={{ color: '#A6762A' }}>{confirmMsg}</div>
          <div className="text-[12px] leading-relaxed mb-2.5" style={{ color: '#A08A5B' }}>
            바뀐 문제로 다시 풀게 하려면 그대로 진행하고, 답안을 지키려면 취소하세요.
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => setConfirmMsg('')}
              className="flex-1 rounded-xl py-2 text-[14px] font-bold"
            ...
            <button
              ...
              disabled={saving}
              className="flex-1 rounded-xl py-2 text-[14px] font-bold text-white disabled:opacity-40"
