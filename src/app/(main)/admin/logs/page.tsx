  ...
  return (
    <div className="px-4 pt-6 pb-24 mx-auto max-w-[860px]">
      <div className="flex items-center gap-3 mb-2">
        <button onClick={() => router.push('/')} className="text-sm" style={{ color: 'var(--color-text-sub)' }}>
          ← 대시보드
        </button>
      ...
      {denied && (
        <div className="rounded-2xl p-4 text-sm" style={{ background: '#FFF1F0', color: '#C0392B' }}>
          기록을 읽을 권한이 없습니다.
        </div>
      )}
      ...
      {suspicious.length > 0 && (
        <div className="rounded-2xl p-4 mb-4" style={{ background: '#FFF6E5', border: '1px solid #F0D9A8' }}>
          <div className="text-sm font-bold mb-1.5" style={{ color: '#8A6D2F' }}>
            ⚠️ 여러 곳에서 접속한 계정
          </div>
          ...
          <div className="text-[12px] mt-2" style={{ color: '#A08A5B' }}>
      ...
      {filterUid && (
        <button
          ...
          className="mb-3 rounded-full px-3 py-1.5 text-[13px] font-bold"
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-[11px] shrink-0" style={{ color: 'var(--color-text-sub)' }}>
                    {l.role}
                  </span>
                </div>
                <span className="text-[12px] shrink-0" style={{ color: 'var(--color-text-sub)' }}>
                  {l.createdAt ? l.createdAt.toLocaleString('ko-KR') : ''}
                </span>
              </div>
              <div className="text-[13px] mt-0.5" style={{ color: 'var(--color-text-main)' }}>
                {l.action}
                {l.classId && ` · ${l.classId}반`}
                {l.detail && ` · ${l.detail}`}
              </div>
              <div className="text-[12px] mt-0.5 font-mono" style={{ color: 'var(--color-text-sub)' }}>
      ...
      <p className="text-[12px] mt-4 leading-relaxed" style={{ color: 'var(--color-text-sub)' }}>
