 ...
 * ---
 *
 * `general`(일반)은 **학교에 속하지 않은 사람**이다.
 *
 * 처음에는 이곳이 학교 하나의 전시실이었다. 그래서 가입하는 사람은 전부
 * 학교 사람이라고 보고, 역할을 학생·선생님·학교관리자·학부모 넷으로만 두었다.
 * 그런데 **학교와 상관없는 전시실**(개인 전시관)이 생기면서, 어느 학교도
 * 고를 수 없는 사람이 가입할 길이 없어졌다 — 넷 중 하나를 억지로 골라야 했고
 * 무엇을 고르든 거짓이었다.
 *
 * 그래서 가입을 **학교 관계자 / 일반** 둘로 먼저 가른다.
 * - 학교 관계자 → 넷으로 다시 갈린다(선생님·학교관리자는 승인 대기).
 * - 일반 → **바로 들어온다.** 기다릴 이유가 없다. 학교 명부를 볼 수 있는 것도
 *   아니고, 반에 들어가는 것도 아니다. 자기 전시관을 열고 남의 전시를 볼 뿐이다.
 *
 * **아무 권한도 아니라는 뜻이 아니다.** `role` 이 `null` 인 계정(가입 도중)과
 * 반드시 구별되어야 한다 — `null` 은 아직 정하지 않은 것이고, `general` 은
 * "학교 사람이 아님" 이라고 정해진 것이다.
 */
export type UserRole = 'super_admin' | 'school_admin' | 'teacher' | 'student' | 'parent' | 'general';
