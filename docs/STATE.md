...
2. **다중접속** (#20) — ⚠️ **사람이 한 번 눌러야 시작할 수 있다.**
   Firebase 콘솔에서 Realtime Database 를 만들어야 한다(서비스 계정 권한으로는
   `firebasedatabase.googleapis.com` 을 켤 수 없다 — 실제로 403 을 받았다).
   만든 뒤 `NEXT_PUBLIC_FIREBASE_DATABASE_URL` 을 .env.local 과 Vercel 에 넣는다.
   **Firestore 금지**(25명 40분 수업에 약 $10). RTDB 사용 +
