 ...
 */
const empty = new Set();
ok('맨 처음에도 할 수 있는 심부름이 있다', openQuests(empty).length >= 1);
...
/** 다 하면 뱃지도 다 모여야 한다 */
ok('다 하면 뱃지를 다 모은다', badgesOf(finished).length === QUESTS.filter((q) => q.badge).length);
ok('다 하면 마을 박사가 된다', rankOf(doneQuests(finished).length).label === '마을 박사');
ok('다 하면 다음 등급이 없다', toNextRank(doneQuests(finished).length) === null);
ok('다 하면 할 일이 없다', openQuests(finished).length === 0);

// 에피소드가 다 채워진다
for (const ch of CHAPTERS) {
  ok(`${ch.id}: 심부름이 둘 이상`, QUESTS.filter((q) => q.chapter === ch.id).length >= 2);
  ok(`${ch.id}: 다 하면 완성된다`, chapterProgress(ch.id, finished).complete);
  ok(`${ch.id}: 처음엔 안 채워져 있다`, chapterProgress(ch.id, empty).done === 0);
}
...
// 사람마다 다른 심부름을 준다
const townQuests = questsAtPlace('townhall', finished);
...
const who = questOfPerson('townhall', 2, readyOnly);
